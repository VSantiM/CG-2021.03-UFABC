...
void OpenGLWindow::handleEvent(SDL_Event& ev) {
  if (ev.type == SDL_KEYDOWN) {
    if (ev.key.keysym.sym == SDLK_UP || ev.key.keysym.sym == SDLK_w) {
      if (m_dollySpeed == 0.0f) { //saindo da inercia
        m_dollySpeed = 1.0f;
      } else if (m_dollySpeed < 4.0f) { //acelerando
        m_dollySpeed = m_dollySpeed + 0.6f;
      } else if (m_dollySpeed >= 4.0f) {}
    } //do nothing
    if (ev.key.keysym.sym == SDLK_DOWN || ev.key.keysym.sym == SDLK_s)
      m_dollySpeed = m_dollySpeed - 0.1f;
...
void OpenGLWindow::initializeGL() {
  ...
  // Load model
  loadModelFromFile(getAssetsPath() + "solar_system.obj");
